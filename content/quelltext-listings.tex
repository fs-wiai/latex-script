...
Nach der erfolgreichen Installation solltet ihr in der Eingabeaufforderung\footnote{Navigation unter Windows: \faWindows\ + R → \enquote{cmd} eingeben → Enter} \mintinline{bash}{python --version} oder \mintinline{bash}{python3 --version} eingeben und mit Enter bestätigen können und in etwa folgendes Ergebnis erhalten:
